...
/**
 * Props:
 * word, focusChar, interactive, background, front, children, scrollLength,
 * fontFamily, fontWeight, annotations, enterLabel, className, style, onProgress
 */
...
export default function GlyphPortal({
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <style>{`
        ${q}{--gp-paper:#000;--gp-ink:#ffffff;--gp-field:#FF0020;--gp-foreground:#ffffff;position:relative;isolation:isolate;background:var(--gp-paper);color:var(--gp-ink);font-family:Arial,sans-serif;}
        ...
        ${q} [data-gp-select]{min-height:44px;min-width:90px;border:1px solid #3a0008;border-radius:4px;background:var(--gp-paper);color:var(--gp-ink);padding:0 10px;font:inherit;}
      ...
      <div data-gp-pin>
        <div data-gp-field aria-hidden="true" inert>
          {background ?? <div data-gp-default-field style={{ position: "absolute", inset: 0, transform: "scale(var(--gp-field-scale,1))", background: "radial-gradient(circle at 18% 8%, rgba(255,0,32,.65), transparent 34%), radial-gradient(circle at 82% 20%, rgba(251,251,250,.12), transparent 28%), radial-gradient(circle at 48% 78%, rgba(120,0,16,.55), transparent 44%), linear-gradient(135deg,#1a0006 0%,#3d000d 48%,#0d0002 100%)" }} />}
        </div>
